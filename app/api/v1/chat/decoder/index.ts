...
import decoderDef from '@/app/public/chat/decoder.json'
...
async function decoder({ embedding, workflow }: { embedding: number[], workflow: string[] }) {
  const req = await fetch(`${decoderDef.url}:predict`, {
    method: 'POST',
    body: JSON.stringify({
      instances: [{
        embedding,
  ...
  decoderDef.vocab.workflow.forEach((workflow, i) => {
...
async function findAnswers(messages: { role: string, content: string }[]) {
  ...
  for (let i = 0; i < 10; i++) {
    const scores = await decoder({ embedding, workflow: previousComponents.map(component => component.type) })
