...
import { ChatGPTEmbeddingCLF } from '@/app/api/client'
...
export default function Chat() {
  ...
  const { trigger } = useAPIMutation(ChatGPTEmbeddingCLF)
  ...
  return (
    <Layout>
      ...
      <main className="flex-grow container mx-auto p-4 flex flex-col gap-6 justify-end">
        <Message role="welcome" session={session} submit={submit} state={state} setState={setState}>
        ...
        {chat.messages.map((message, i) =>
          <Message key={i} role={message.role} session={session} submit={submit} state={state} setState={setState}>{message.content}</Message>
        )}
        {chat.waitingForReply ?
          <Message role="assistant" session={session} submit={submit} state={state} setState={setState}>
            <progress className="progress w-full"></progress>
          </Message>
          : null}
        <Message role="user" session={session} submit={submit} state={state} setState={setState}>
          <form
