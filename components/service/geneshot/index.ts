  ...
  [
    ...
    MetaNode(`GeneshotTermSearch[${T.name}, ${rc}]`)
      ...
      .story(props => ({
        ...
        introduction: `PubMed contains millions of abstracts containing genes and other biomedical terms such as drugs or diseases. Geneshot is a web server that facilitates arbitrary term searches against PubMed to find gene co-mentions & gene set augmentation with a variety of gene-gene similarity resources\\ref{doi:10.1093/nar/gkz393}.`,
        methods: `The term was queried against PubMed, and genes co-mentioned in the resulting abstracts were found and ranked by occurrence based on ${label} using Geneshot\\ref{doi:10.1093/nar/gkz393}.`,
        tableLegend: `A table of the normalized genes co-occurring with the term from PubMed abstracts identified using ${label} with Geneshot\\ref{doi:10.1093/nar/gkz393}.`,
      }))
      .build()
  ])
)
...
export const GeneshotGeneSetAugmentation = ([
  ...
  MetaNode(`GeneshotGeneSetAugmentation[${rc}]`)
    ...
    .story(props => ({
      ...
      tableLegend: `A table of similar genes to ${props.input_refs?.geneset} produced by using a ${label} similarity matrix with Geneshot\\ref{doi:10.1093/nar/gkz393}.`,
    }))
    .build(),
  MetaNode(`GeneshotGeneAugmentation[${rc}]`)
    ...
    .story(props => ({
      ...
      tableLegend: `A table of similar genes to ${props.inputs?.gene ? props.inputs.gene : `${props.input_refs?.gene}`} produced by using a ${label} similarity matrix with Geneshot\\ref{doi:10.1093/nar/gkz393}.`,
    }))
    .build(),
])
