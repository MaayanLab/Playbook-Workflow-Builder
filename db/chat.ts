...
import { json_safe_timestamp_codec, nullable_undefined_codec, z_uuid } from '@/utils/zod'
...
export const thread = Table.create('thread')
  ...
  .field('openai_thread', 'varchar', '', nullable_undefined_codec(z.string()), { default: () => null })
...
export const thread_message = Table.create('thread_message')
  .field('id', 'uuid', 'default uuid_generate_v4()', z_uuid(), { primaryKey: true, default: uuidv4 })
  .field('fpl', 'uuid', 'references "fpl" ("id") on delete cascade', nullable_undefined_codec(z_uuid()), { default: () => null })
  ...
  .field('feedback', 'varchar', '', nullable_undefined_codec(z.string()), { default: () => null })
  .field('created', 'timestamp without time zone', 'not null default now()', json_safe_timestamp_codec(), { default: () => new Date() })
  .build()
